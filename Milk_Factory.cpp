...
// URL: https://vjudge.net/problem/USACO-940  
// Start: 8/24/2024, 4:26:34 PM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
void UseFile() { freopen("factory.in", "r", stdin); freopen("factory.out", "w", stdout); }

int countChildren(int i, vvi &graph){
    int count = graph[i].size();
    for(int j : graph[i]) count += countChildren(j, graph);
    return count;
}
...
/*
NOTES:
N stations (in a tree directional)
print the (minimal i) station that
can be reached from all other stations 
if there's not print -1
*/
