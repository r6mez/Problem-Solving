...
// Memory Limit: 256
// Time Limit: 1500
// Start: Thu Aug 29 16:03:49 2024 
#ifdef ALGOAT
#include "debug.hpp"
#else
#define debug(...) 0
...
int main() {
    ...
    while (t--) {
        ...
        while(k--){
            ...
            x--; y--;
            countOp[x]++;
            if(y+1 < m) countOp[y+1]--;
        }

        partial_sum(all(countOp), countOp.begin());

        vll countAdd(n);
        for (int i = 0; i < m; i++) {
            countAdd[op[i][0]] += op[i][2] * countOp[i];
            if(op[i][1]+1 < n) countAdd[op[i][1]+1] -= op[i][2] * countOp[i];
        }

        partial_sum(all(countAdd), countAdd.begin());
