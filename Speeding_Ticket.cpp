...
int main() {
    ...
    while (t--) {
        int n, m; cin >> n >> m;
        vector<pii> road(n), cow(m);
        for (int i = 0; i < n; i++) {
            cin >> road[i].first >> road[i].second;
            if(i > 0) road[i].first += road[i-1].first;
        }
        for (int i = 0; i < m; i++) {
            cin >> cow[i].first >> cow[i].second;
            if(i > 0) cow[i].first += cow[i-1].first;
        }

        vi speedLimit(105), speed(105);
        int k = 0, j = 0;
        for (int i = 1; i <= 100; i++){
            if (road[k].first >= i) speedLimit[i] = road[k].second;
            else k++, speedLimit[i] = road[k].second;
            if (cow[j].first >= i) speed[i] = cow[j].second;
            else j++, speed[i] = cow[j].second;
        }

        int maxDiff = 0;
        for (int i = 1; i <= 100; i++) {
            maxDiff = max(maxDiff, speed[i] - speedLimit[i]);
        }
