...
// URL: https://vjudge.net/problem/USACO-965  
// Start: 8/17/2024, 5:10:31 PM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
void UseFile() { freopen("lineup.in", "r", stdin); freopen("lineup.out", "w", stdout); }
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            cin >> s[i][0];
            string uselessShit; for (int i = 0; i < 4; i++) cin >> uselessShit;
            cin >> s[i][1];
            sort(all(s[i]));
        }

        int c = 8;
        vs cows = { "Beatrice", "Belinda", "Bella", "Bessie", "Betsy", "Blue", "Buttercup", "Sue" };
        ...
        do {
            ...
            for (int i = 0; i < c - 1; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    if (min(cows[i], cows[i + 1]) == s[j][0] && max(cows[i], cows[i + 1]) == s[j][1])
                        counter++;
                }
            }
