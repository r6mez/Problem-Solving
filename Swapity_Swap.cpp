...
// URL: https://vjudge.net/problem/USACO-1013  
// Start: 8/24/2024, 5:36:27 PM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
void UseFile() { freopen("swap.in", "r", stdin); freopen("swap.out", "w", stdout); }
...
int main() {
    ...
    FastIO();
    int n, k; cin >> n >> k;
    int a1, a2, b1, b2, counter = 0;
    cin >> a1 >> a2 >> b1 >> b2;
    a1--; a2--; b1--; b2--;

    vi order(n);
    iota(order.begin(), order.end(), 1);
    vi original_order = order;
    do {
        reverse(order.begin() + a1, order.begin() + a2 + 1); // reverse[starting_position, ending_position)
        reverse(order.begin() + b1, order.begin() + b2 + 1);
        counter++;

    } while (original_order != order);

    k %= counter;

    while (k--) {
        reverse(order.begin() + a1, order.begin() + a2 + 1);
        reverse(order.begin() + b1, order.begin() + b2 + 1);
    }
