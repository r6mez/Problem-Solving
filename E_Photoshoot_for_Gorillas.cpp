...
int main() {
    ...
    while (t--) {
        ...
        vector<vll> b = prefixSum2D(a);

        priority_queue<ll, vll> squares, gorillas;
        ...
        while (gorillas.empty() == false) {
            ans += squares.top() * gorillas.top();
            squares.pop(); gorillas.pop();
        }
