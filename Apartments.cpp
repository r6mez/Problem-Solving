...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < m; i++){
            auto it = lower_bound(a.begin()+lastTakenIndex, a.end(), p[i]-k);
            int nearestPersonIndex = it - a.begin();
            if(a[nearestPersonIndex] <= p[i]+k && a[nearestPersonIndex] >= p[i]-k) {
                counter++;
                lastTakenIndex = nearestPersonIndex+1;
            }
        }
        cout << counter << "\n";
    }
