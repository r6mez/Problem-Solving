...
// Memory Limit: 256
// Time Limit: 2000
// Start: Tue 17 Sep 2024 04:05:47 PM EEST 
#include <cmath>
#include <functional>
#include <iomanip>
#ifdef RAMEZ
...
vll getDivisors(ll n); bool isPrime(ll n); vector<pll> getPrimeFactors(ll n); set<ll> linearSeive(ll n);
...
vector<pll> getPrimeFactors(ll n){
  ...
  for (ll i = 2; i*i <= n; i++) {
    if(n%i == 0) {
      ...
      primeFactors.push_back({n,count}); 
    } 
  }
...
set<ll> linearSeive(ll n){
  vll isPr(n, 1); set<ll> primes;
  isPr[0] = isPr[1] = 0;

  for (ll i = 2; i < n; i++) {
    if(isPr[i]) primes.emplace(i);
    for(ll p : primes){
      if(i * p >= n) break;
      isPr[i*p] = 0;
      if(i%p == 0) break;
