...
// Memory Limit: 256
// Time Limit: 2000
// Start: Sun 27 Oct 2024 06:19:46 PM EEST 
#ifdef RAMEZ
#include "debug.hpp"
#else
#define debug(...) 0
...
int flipZeroBeforeLastOne(int num) {
    ...
    for (int i = lastOnePosition - 1; i >= 0; i--) {
        if (num & (1 << i)) {
            num &= ~(1 << i);
            break;
        }
...
int getSmallerPowerOfTwo(int num) {
    int lastOnePosition = -1;
    for (int i = 31; i >= 0; i--) {
        if (num & (1 << i)) {
            lastOnePosition = i;
    ...
    int smallerPowOfTwo = 1 << (lastOnePosition);
    return smallerPowOfTwo;
}
...
void Ramez(){
  ...
  if(n%2 == 1){
    ...
    int a = n;
    int b = flipZeroBeforeLastOne(a);
    int c = getSmallerPowerOfTwo(a)-1;
...
/*
NOTES:
What matter the most is the last 3 ops
if n is even 
  or and or 
  the last 3 elements is ( n)

if n is Odd
  and or and
  last element matter the most (n)
  other elements doesn't matter
*/
