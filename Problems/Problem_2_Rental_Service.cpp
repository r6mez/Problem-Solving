...
// Memory Limit: 256
// Time Limit: 4000
// Start: Sat Aug 31 17:51:04 2024 
#include <functional>
#ifdef ALGOAT
#include "debug.hpp"
#else
...
void UseFile() { freopen("rental.in", "r", stdin); freopen("rental.out", "w", stdout); }
...
int main() {
    ...
    while (t--) {
        ...
        ll maxMoney = 0;
        while(cowAt < n){
            int amount = cows[cowAt];
            int soldMoney = 0;
            int currShop = shopAt;
            int last = 0;

            while(currShop < m){
                int sold = min(amount, shops[currShop].second);
                soldMoney += shops[currShop].first * sold;
                amount -= sold;
            ...
            if(rentAt >= r || soldMoney > rents[rentAt]){ // buy
                maxMoney += soldMoney;
                shopAt = currShop;
                if(shopAt < m) shops[shopAt].second -= last;
                cowAt++;
            } else { // rent 
