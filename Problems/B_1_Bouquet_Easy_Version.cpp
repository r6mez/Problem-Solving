...
const int MOD = 1000000000000000000;
...
void solve() {
    ...
    for (auto [warda1, freq1] : flowers) {
        auto [warda2, freq2] = make_pair((warda1 - 1), flowers[warda1 - 1]);
        ll sum = (warda1 * freq1) + (warda2 * freq2);
        if (sum > k) {
       
            for (int i = 0; i <= freq1; ++i) {
                ll remCapacity = k - (i * warda1);
                if (remCapacity < 0) continue;
                ll usedWarda2 = min(freq2, remCapacity / warda2);
                ll currentSum = i * warda1 + usedWarda2 * warda2;
