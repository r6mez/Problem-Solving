...
// ----------------------------------------------------
// Contest: Codeforces Round 587 (Div. 3)
// Judge: Codeforces
// URL: https://codeforces.com/contest/1216/problem/C
// Memory Limit: 256
// Time Limit: 1000
// Start: Mon Aug 26 23:10:13 2024
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
pair<pii, pii> intersect(pii a, pii b, pii c, pii d) {
    ...
    if(r <= l or u <= dw) return {{0,0},{0,0}};
    else return {{l,dw}, {r,u}};
}
...
int main() {
    ...
    while (t--) {
        ...
        pair<pii, pii> bb = intersect(wb1.x, wb1.y, wb2.x, wb2.y);

        ll wArea = area(p[0], p[1]);
        ll wb1Area = area(wb1.x, wb1.y);
        ll wb2Area = area(wb2.x, wb2.y);
        ll bbArea = area(bb.x, bb.y);

        if(wArea > wb1Area + wb2Area - bbArea) cout << "YES\n";
        else cout << "NO\n";
