...
// ----------------------------------------------------
// Contest: ITMO Academy: pilot course - Binary Search - Step 3
// Judge: Codeforces
// URL: https://codeforces.com/edu/course/2/lesson/6/3/practice/contest/285083/problem/A
// Memory Limit: 256
// Time Limit: 1000
// Start: Tue 10 Sep 2024 04:33:07 PM EEST 
#include <iomanip>
#ifdef RAMEZ
#include "debug.hpp"
#else
...
pair<double, double> intersect(double l1, double r1, double l2, double r2){
...
bool can(double t, vector<double> x, vector<double> v) {
    double l_common = -1e18, r_common = 1e18; 
    
    for (int i = 0; i < x.size(); i++) {
        double l = x[i] - (v[i] * t); 
        double r = x[i] + (v[i] * t);  
        
        pair<double, double> common = intersect(l_common, r_common, l, r);
        
        if (common.first == -1) {
            return false;
        }
        
        l_common = common.first;
        r_common = common.second;
    }
