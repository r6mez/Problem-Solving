...
// Memory Limit: 256
// Time Limit: 3000
// Start: Sun 22 Sep 2024 11:07:55 PM EEST 
#include <cmath>
#include <functional>
#ifdef RAMEZ
#include "debug.hpp"
...
void solve() {
    ...
    vi remainderCount(3,0);  // Array to track if remainder (0, 1, 2) has been seen.
    int r = 0, s = 0;  // 'r' to count valid numbers, 's' to track running sum of digits.
    
    remainderCount[0] = 1;  // Initialize for remainder 0 to handle cases when the sum starts divisible by 3.

    for (char c : a) {
        // Add the value of the current digit to the running sum
        s += (c - '0'); 
        
        debug(remainderCount);

        // If a segment divisible by 3 can be formed (i.e., remainder is 0)
        if (remainderCount[s % 3] > 0) {
            r++;  // Increase count of valid segments
            s = 0;  // Reset running sum for next segment
            fill(all(remainderCount), 0);  // Reset array
            remainderCount[0] = 1;  // Reinitialize for the next round
        }
        
        // Mark that we encountered this remainder
        remainderCount[s % 3] = 1;
    }
