...
// URL: https://vjudge.net/problem/USACO-687  
// Start: 8/19/2024, 4:31:58 PM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
void UseFile() { freopen("notlast.in", "r", stdin); freopen("notlast.out", "w", stdout); }
...
int main() {
    ...
    vector<int> milkAmounts;
    for (auto &[cow, milk] : cows) {
        milkAmounts.push_back(milk);
    }
    sort(milkAmounts.begin(), milkAmounts.end());

    int minMilk = milkAmounts[0];
    int secondMinMilk = -1;

    // Find the second minimum milk amount
    for (int i = 1; i < milkAmounts.size(); i++) {
        if (milkAmounts[i] > minMilk) {
            secondMinMilk = milkAmounts[i];
            break;
        }
    }

    // Check which cow has the second minimum milk amount
    string minMilkCow = "Tie";
    int count = 0;
    for (auto &[cow, milk] : cows) {
    ...
    // If more than one cow has the second minimum milk amount, it's a tie
    if (count > 1 || secondMinMilk == -1) {
        minMilkCow = "Tie";
    }
