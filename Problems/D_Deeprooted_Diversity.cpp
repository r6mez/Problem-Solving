...
bool comp(pll a, pll b) {
    if (a.first != b.first) 
        return a.first < b.first; // Sort by `b[i]` (first) in descending order
    return a.second >= b.second;   // For equal `b[i]`, sort by `a[i]` (second) in ascending order
}
...
void Ramez() {
    ...
    {
        if(sumA + extra[i].second >= sumB + extra[i].first){
            sumA += extra[i].second;
            sumB += extra[i].first;
            counter++;
        }
