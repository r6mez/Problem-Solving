...
void Ramez() {
    ...
    int layer = 1, piecesReq = 1, piecesDone = 0;
    for (int i = 0; i < n; i++){
        int needed = piecesReq - piecesDone;

        // if the pieces I have is >= needed to finish this layer
        while(a[i] >= needed){
            // finish old layer
            a[i] -= needed;
            // enter new layer
            piecesReq = (layer+2)*2 + layer*2;
            piecesDone = 0;
            layer+=2;
            // recalculate the needed 
            needed = piecesReq - piecesDone;
        } 
