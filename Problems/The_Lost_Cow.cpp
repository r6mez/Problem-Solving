...
// URL: https://vjudge.net/problem/USACO-735  
// Start: 8/2/2024, 11:51:48 AM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
int main() {
    ...
    while (t--) {
        int n, x; cin >> n >> x;
        int sum = 0, pos = n, move = 1;
        while(!((n < x && pos >= x) || (n > x && pos <= x))){
            if(n < x) sum += abs(pos - min(n+move, x));
            else sum += abs(pos - max(n+move, x));
            pos = n + move;
            move *= -2;
        }
        cout << sum << "\n";
