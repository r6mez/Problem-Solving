...
// ----------------------------------------------------
// Contest: Codeforces Beta Round 67 (Div. 2)
// Judge: Codeforces
// URL: https://codeforces.com/problemset/problem/75/C
// Memory Limit: 256
// Time Limit: 2000
// Start: Thu 19 Sep 2024 07:56:52 PM EEST 
#include <cmath>
#include <functional>
#include <iterator>
#ifdef RAMEZ
...
set<ll> getCommonDivisors(ll a, ll b) {
    set<ll> divisors;
    ll gcd_ab = __gcd(a, b);
    for (ll i = 1; i * i <= gcd_ab; i++) {
        if (gcd_ab % i == 0) {
            divisors.insert(i);
            if (i*i != gcd_ab) {
                divisors.insert(gcd_ab / i);
            }
        }
    }
    return divisors;
...
void solve(){
  ll a, b; cin >> a >> b;
  set<ll> commonDivisors = getCommonDivisors(a,b);
  debug(commonDivisors);

  ll n; cin >> n;
  while(n--){
    ll l, r; cin >> l >> r;
    auto it = commonDivisors.lower_bound(l);
    if (it == commonDivisors.end() || *it > r) {
      cout << -1 << "\n";
    } else {
      auto rit = commonDivisors.upper_bound(r);
      if (rit == commonDivisors.begin() || (--rit, *rit < l)) {
        cout << -1 << "\n";
      } else {
        cout << *rit << "\n";
      }
