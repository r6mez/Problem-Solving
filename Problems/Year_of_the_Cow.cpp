...
// URL: https://vjudge.net/problem/USACO-1107  
// Start: 8/19/2024, 2:07:39 PM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
int main() {
    ...
    while (t--) {
        ...
        int n; cin >> n;
        map<string, int> cowPositions; // بالنسبة لبيسي 
        map<string, string> cowMonths;
        cowPositions["Bessie"] = 0;
        cowMonths["Bessie"] = "Ox";
        string cow, dir, nextYear, referenceCow, uselessShit;
        int ans = 0;
        for (int i = 0; i < n; i++) {
            cin >> cow;
            cin >> uselessShit >> uselessShit;
            cin >> dir;
            cin >> nextYear;
            cin >> uselessShit >> uselessShit;
            cin >> referenceCow;

            cowPositions[cow] = cowPositions[referenceCow];
            string prevYear = cowMonths[referenceCow];

            if (dir == "next") {
                if (years[nextYear] <= years[prevYear]) {
                    cowPositions[cow] += (12 - years[prevYear]) + (years[nextYear]);
                }
                else {
                    cowPositions[cow] += (years[nextYear] - years[prevYear]);
                }
            }
            else {
                if (years[nextYear] < years[prevYear]) {
                    cowPositions[cow] -= (years[prevYear] - years[nextYear]);
                }
                else {
                    cowPositions[cow] -= (years[prevYear]) + (12 - years[nextYear]);
                }
            }
            cowMonths[cow] = nextYear;
            // cout << cowPositions[cow] << "\n";
        }

        cout << abs(cowPositions["Elsie"]) << "\n";
    }
    return 0;
}
...
Gretta previous Monkey Mildred
Elsie next Ox Gretta
Paulina next Dog Bessie
