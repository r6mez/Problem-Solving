...
// URL: https://vjudge.net/problem/USACO-831  
// Start: 8/3/2024, 1:23:15 PM
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
set<char> winingCows;
set<pair<char,char>> winingTeams;

void checkCow(char a, char b, char c) {
    if(a == b && b == c) winingCows.insert(a);
}

void checkTeam(char a, char b, char c) {
    set<char> s = { a, b, c };
    if(s.size() == 2) winingTeams.insert({*s.begin(), *next(s.begin())});
}
...
int main() {
    ...
    // cin >> t;
    while (t--) {
        vector<string> game(3); cin >> game;

        int alone = 0;
        int team = 0;

        checkCow(game[0][0], game[0][1], game[0][2]);
        checkCow(game[1][0], game[1][1], game[1][2]);
        checkCow(game[2][0], game[2][1], game[2][2]);
        checkCow(game[0][0], game[1][0], game[2][0]);
        checkCow(game[0][1], game[1][1], game[2][1]);
        checkCow(game[0][2], game[1][2], game[2][2]);
        checkCow(game[0][0], game[1][1], game[2][2]);
        checkCow(game[0][2], game[1][1], game[2][0]);

        checkTeam(game[0][0], game[0][1], game[0][2]);
        checkTeam(game[1][0], game[1][1], game[1][2]);
        checkTeam(game[2][0], game[2][1], game[2][2]);
        checkTeam(game[0][0], game[1][0], game[2][0]);
        checkTeam(game[0][1], game[1][1], game[2][1]);
        checkTeam(game[0][2], game[1][2], game[2][2]);
        checkTeam(game[0][0], game[1][1], game[2][2]);
        checkTeam(game[0][2], game[1][1], game[2][0]);

        cout << winingCows.size() << "\n" << winingTeams.size() << "\n";
    }
    return 0;
}
