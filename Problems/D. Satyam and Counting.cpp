...
// Memory Limit: 256
// Time Limit: 2000
// Start: Tue 03 Sep 2024 05:57:04 PM EEST
#include <iterator>
#ifdef ALGOAT
#include "debug.hpp"
#else
...
int main() {
  ...
  while (t--) {
    ...
    for (int i = 0; i < a.size(); i++) {
        if(as.find(a[i]+2) != as.end() && bs.find(a[i]+1) != bs.end()) tringles++;
    }

    for (int i = 0; i < b.size(); i++) {
        if(bs.find(b[i]+2) != bs.end() && as.find(b[i]+1) != as.end()) tringles++;
    }
...
/*
NOTES:
any two point above each other makes = greater lower points + greater upper
points any three consecutive points 1,2,3 that the middle is diffrenet makes = 1
