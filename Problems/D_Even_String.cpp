/*
    I thought I could bring an end to the world's suffering,
    But when every equation was solved all that remained
    were fields of dreamless solitude.
*/
// D. Even String
// URL: https://codeforces.com/contest/2086/problem/D
// Time: 4/3/2025, 5:24:22 PM
#include <bits/stdc++.h>
using namespace std;
#define int    long long
#define vi     vector<int>
#define pii    pair<int, int>
#define all(v) v.begin(), v.end()
template<typename T> ostream& operator<<(ostream& os, vector<T>& v) { for (auto& i : v) os << i << ' '; return os; }
template<typename T> istream& operator>>(istream& is, vector<T>& v) { for (auto& i : v) is >> i; return is; }
void FastIO() { ios_base::sync_with_stdio(false); cin.tie(nullptr); }
void UseFile() { freopen("file.in", "r", stdin); freopen("file.out", "w", stdout); }
const int MOD = 998244353;

int add(int a, int b) { return ((a % MOD) + (b % MOD)) % MOD; }
int mul(int a, int b) { return ((a % MOD) * (b % MOD)) % MOD; }
int sub(int a, int b) { return ((((a % MOD) - (b % MOD)) % MOD) + MOD) % MOD; }


int pwmod(int b, int p) {
    if (!p) return 1LL;
    int ret = pwmod(b, p >> 1LL);
    ret = mul(ret, ret);
    if (p & 1LL)
        ret = mul(ret, b);
    return ret;
}

const int SizeMax = 1e6 + 2;

vi fac(0), inv(0);

void buildFacAndInvFac() {
    fac.resize(SizeMax);
    inv.resize(SizeMax);
    fac[0] = 1;
    for (int i = 1; i < SizeMax; ++i) {
        fac[i] = mul(fac[i - 1], i);
    }
    inv[0] = 1;
    for (int i = 1; i < SizeMax; ++i) {
        inv[i] = pwmod(fac[i], MOD - 2);
    }
}

int nCr2(int n, int r) {
    if (r > n) return 0;
    return mul(mul(fac[n], inv[n - r]), inv[r]);
}

void Ramez() {
    vi s(26); cin >> s;
    
    int n = accumulate(all(s), 0LL);
    int oddCount = (n + 1) / 2, evenCount = n / 2;

    bool possible = true;
    for (int i = 0; i < 26; i++){
        if(s[i] > (n + 1) / 2){
            cout << 0 << "\n";
            return;
        }
    }

    vi dp(oddCount + 1, 0); dp[0] = 1; 
    for (int i = 0; i < 26; i++){
        if(s[i] == 0) continue;
        for (int j = oddCount; j >= s[i]; j--){
            dp[j] = (dp[j] + dp[j - s[i]]) % MOD;
        }
    }

    int ways = (fac[oddCount] * fac[evenCount]) % MOD;
    for (int i = 0; i < 26; i++){
        if(s[i] == 0) continue;
        ways = (ways * inv[s[i]]) % MOD;
    }

    int answer = (dp[oddCount] * ways) % MOD;
    cout << answer << "\n";
}

/*
NOTES:

*/

int32_t main() {
    // UseFile();
    FastIO();

    buildFacAndInvFac();

    int t = 1;
    cin >> t;
    while (t--) Ramez();
    return 0;
}