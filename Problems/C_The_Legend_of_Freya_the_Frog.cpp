...
// Memory Limit: 256
// Time Limit: 2000
// Start: Tue 03 Sep 2024 05:47:22 PM EEST 
#ifdef ALGOAT
#include "debug.hpp"
#else
#define debug(...) 0
...
int main() {
    ...
    while (t--) {
        ...
        ll xMoves = ceil(x/k), yMoves = ceil(y/k);
        ll moves = xMoves + yMoves;
        if(yMoves >= xMoves) moves += yMoves-xMoves;
        else moves += xMoves-yMoves-1;
        cout << moves << "\n";
    }
    return 0;
}
