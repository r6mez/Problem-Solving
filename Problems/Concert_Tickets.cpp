...
int main() {
    ...
    while (t--) {
        int n, m; cin >> n >> m;
        vi tickets(n), persons(m); cin >> tickets >> persons;
        multiset<int> available_tickets(all(tickets));

        for (int i = 0; i < m; i++) {
            auto it = available_tickets.upper_bound(persons[i]);
