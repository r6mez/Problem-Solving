...
// Memory Limit: 256
// Time Limit: 2000
// Start: Sat 02 Nov 2024 05:19:12 PM EET 
#ifdef RAMEZ
#include "debug.hpp"
#else
#define debug(...) 0
...
pair<int, int> nextEle(int i, int j, vector<string> &s) {
    int layer = min(min(i, j), min(n - i - 1, m - j - 1));
    ...
    if (i == top && j < right) return {i, j + 1}; // Move right
    if (j == right && i < bottom) return {i + 1, j}; // Move down
    if (i == bottom && j > left) return {i, j - 1}; // Move left
    if (j == left && i > top) return {i - 1, j}; // Move up

    if (i == top && j == left) return {i, j + 1}; // Move right
    if (i == bottom && j == left) return {i - 1, j}; // Move up
    if (i == top && j == right) return {i + 1, j}; // Move down
    if (i == bottom && j == right) return {i, j - 1}; // Move left

    return {-1, -1}; // Invalid position if no valid next element
}
...
void Ramez() {
    ...
    for (int i = 0; i < n / 2; i++) {
        for (int j = 0; j < m; j++) {
            if (s[i][j] == '1') {
                auto next = nextEle(i, j, s);
                auto secondNext = nextEle(next.first, next.second, s);
                auto thirdNext = nextEle(secondNext.first, secondNext.second, s);
                if (s[next.first][next.second] == '5' && s[secondNext.first][secondNext.second] == '4' && 
                    s[thirdNext.first][thirdNext.second] == '3') {
                    counter++;
                }
            }
        }
