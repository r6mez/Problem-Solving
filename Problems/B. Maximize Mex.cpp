...
// Memory Limit: 256
// Time Limit: 1000
// Start: Sun 06 Oct 2024 09:13:54 AM EEST 
#include <algorithm>
#include <climits>
#ifdef RAMEZ
#include "debug.hpp"
...
void solve(){
  ...
  while(true){
    if(binary_search(all(a), mex)){
      mex++;
    } 
    else {
      auto it = dup.lower_bound({mex%x, 0});
      if(it == dup.end() || it->first != mex%x || it->second >= mex){
        break;
      } else {
        dup.erase(it);
        mex++; 
