...
// Memory Limit: 512
// Time Limit: 1000
// Start: Mon 02 Sep 2024 07:43:27 PM EEST 
#ifdef ALGOAT
#include "debug.hpp"
#else
#define debug(...) 0
...
int main() {
    ...
    while (t--) {
        ...
        s.emplace(0); s.emplace(x);
        answers.emplace(x);
        for (int i = 0; i < n; i++) {
            int p; cin >> p;
            s.emplace(p);
            int next = *s.upper_bound(p);
            int prev = *(--s.lower_bound(p));
            answers.erase(answers.find(next-prev));
            answers.emplace(next-p);
            answers.emplace(p-prev);
            cout << *answers.rbegin() << " "; 
        }
    }
    return 0;
