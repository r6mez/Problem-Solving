...
void processNighbours(int& n, int& m, int& countGood, queue<pii> &q, vs &a, vvi& good) {
    while (!q.empty()) {
        auto [x, y] = q.front(); q.pop();
        // process nighbours
        if (x + 1 < n && a[x + 1][y] == 'U' && good[x + 1][y] == 0) {
            good[x + 1][y] = 1;
            q.push({ x + 1, y });
            countGood++;
        }

        if (x - 1 >= 0 && a[x - 1][y] == 'D' && good[x - 1][y] == 0) {
            good[x - 1][y] = 1;
            q.push({ x - 1, y });
            countGood++;
        }

        if (y + 1 < m && a[x][y + 1] == 'L' && good[x][y + 1] == 0) {
            good[x][y + 1] = 1;
            q.push({ x, y + 1 });
            countGood++;
        }

        if (y - 1 >= 0 && a[x][y - 1] == 'R' && good[x][y - 1] == 0) {
            good[x][y - 1] = 1;
            q.push({ x, y - 1 });
            countGood++;
        }
...
// function that find all the good cells
void findGood(int& n, int& m, int& countGood, vs& a, vvi& good) {
    for (int i = 0; i < n; i++) {
        if (a[i][0] == 'L') {
            good[i][0] = 1;
            queue<pii> q;
            q.push({ i, 0 });
            countGood++;
            processNighbours(n, m, countGood, q, a, good);
        }
...
// It has at least one bad nighbour 
void countMarksThatBecomesGood(int& n, int& m, int& countGoodMarks, vs& a, vvi& good){
    for (int i = 0; i < n; i++){
        for (int j = 0; j < m; j++){
            if(a[i][j] == '?'){
                if(i+1 < n && good[i+1][j] == 0);
                else if(i-1 >= 0 && good[i-1][j] == 0);
                else if(j+1 < m && good[i][j+1] == 0);
                else if(j-1 >= 0 && good[i][j-1] == 0);
                else {good[i][j] = 1; countGoodMarks++;}
            }
        }
    }
}
...
void Ramez() {
    ...
    int countGood = 0;
    findGood(n, m, countGood, a, good);
    
    int countGoodMarks = 0;
    countMarksThatBecomesGood(n, m, countGoodMarks, a, good);
    
    int countBad = (n * m) - countGood - countGoodMarks;
