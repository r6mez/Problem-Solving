...
// Memory Limit: 512
// Time Limit: 1000
// Start: Wed 04 Sep 2024 10:19:15 PM EEST 
#ifdef RAMEZ
#include "debug.hpp"
#else
#define debug(...) 0
...
int main() {
    ...
    while (t--) {
      ...
      int moviesCount = 0;
      for (int j = 0; j < n; j++) {
        auto it = s.upper_bound(movies[j].S);
        if(it == s.begin()) continue; // all of them missed this movie 
        
        s.erase(--it); s.emplace(movies[j].F);
        moviesCount++;
      }
