...
// Contest: Contest #2
// Judge: Codeforces
// URL: https://codeforces.com/group/ftB9bVYh2W/contest/561236/problem/E
// Memory Limit: 256
// Time Limit: 1000
// Start: Fri 25 Oct 2024 05:19:05 PM EEST 
#include <algorithm>
#ifdef RAMEZ
#include "debug.hpp"
#else
...
void Ramez(){
  ...
  for (int i = 0; i < s.size(); i++) {
    if(s[i] == 'B'){
      if(!upper.empty()) upper.pop(); 
      continue;
    } 

    if(s[i] == 'b'){
      if(!lower.empty()) lower.pop();
      continue;
    }
  ...
  while(!lower.empty() && !upper.empty()){
    if(upper.top().second > lower.top().second){
      result.push_back(upper.top().first);
      upper.pop();
    } else {
      result.push_back(lower.top().first);
      lower.pop();
    }
  }
