...
// Memory Limit: 512
// Time Limit: 2000
// Start: Wed 18 Sep 2024 08:43:37 PM EEST 
#include <cmath>
#include <functional>
#ifdef RAMEZ
#include "debug.hpp"
...
vll getDivisors(ll n); bool isPrime(ll n); void getPrimeFactors(ll n, ll freq, vll &prefix); vll linearSeive(ll n);
...
void solve(){
  ...
  while(m--){
    ll l, r; cin >> l >> r;
    ll ans = freq[min(N-1, r)] - freq[min(N-1, l-1)];
    cout << ans << "\n";
  }
}
...
void getPrimeFactors(ll n, ll freq, vll &prefix){
  for (ll i = 2; i*i <= n; i++) {
    if(n%i == 0) {
      while(n%i == 0) n /= i;
      prefix[i] += freq;
