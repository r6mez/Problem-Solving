...
// Memory Limit: 256
// Time Limit: 4000
// Start: Wed 25 Sep 2024 09:59:34 PM EEST 
#include <cmath>
#include <functional>
#include <numeric>
#ifdef RAMEZ
...
void solve(){
  ll n, m, k; cin >> n >> m >> k;
  vll a(n), frq(n+1), pre(n+1); cin >> a; 
  
  for (int i = 0; i < n; i++) {
    frq[a[i]]++;
  }

  partial_sum(all(frq), pre.begin());

  ll ans = 0;
 
  for (ll i = 1; i <= n; i++) {
    ll sum = pre[min(n, i+k)] - pre[i];
    for (int j = 1; j <= min(m, frq[i]); j++) {
      ans = add(ans, mul(nCr(frq[i], j), nCr(sum, m-j)));
    }
  } 
