...
int main() {
    ...
    while (t--) {
        int n; cin >> n;
        set<int> s;
        float nearstSqrt = sqrt(n-1);
        if(nearstSqrt - int(nearstSqrt) > 0) nearstSqrt = int(nearstSqrt+1);

        vi a(n);
        for (int i = n-1; i >= 0; ){
            if(s.find(nearstSqrt*nearstSqrt - i) == s.end() && (nearstSqrt*nearstSqrt - i < n)){ // not used and less than n 
                a[i] = nearstSqrt*nearstSqrt - i;
                s.insert(nearstSqrt*nearstSqrt - i);
                i--;
            } else {
                nearstSqrt--;
            }
