...
const int MOD = 998244353, INF = 2e18;
...
void solve() {
    ...
    if(a[0] > zeroCnt) finish(0);
    
    int x = zeroCnt - a[0];

    int ans = nCr(zeroCnt, x);
    ans = multiply(ans, fact[x]);
    ans = multiply(ans, fact[n - x]);
