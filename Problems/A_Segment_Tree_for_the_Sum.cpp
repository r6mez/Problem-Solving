...
private:
    ...
    ll sum(int l, int r, int currentNode, int curL, int curR) {
        if (curL >= r || curR <= l) return 0; // don't intersect
        if (curL >= l && curR <= r) return tree[currentNode]; // fully intersect

        int mid = (curL + curR) / 2;
        ll sumLeft = sum(l, r, 2 * currentNode + 1, curL, mid);
        ll sumRight = sum(l, r, 2 * currentNode + 2, mid, curR);
