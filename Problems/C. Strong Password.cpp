...
// Memory Limit: 256
// Time Limit: 2000
// Start: Tue 24 Sep 2024 09:17:04 PM EEST 
#include <cmath>
#include <functional>
...
void solve(){
  ...
  int curr = 0, prev = -1, mx = 0;
  for (int i = 0; i < m; i++) {
    for (char j = l[i]; j <= r[i]; j++) {
      curr = s.find(j, prev+1);
      if(curr == string::npos) {
        cout << "YES\n";
        return;
      }
...
/*
NOTES:
you can't store all subsequences duo to huge |S| = 1e5 * 3
so ... we must check being a subsequece a few times 

checking all numbers between l and r might be 
10^9 also not achiveable
