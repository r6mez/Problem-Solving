...
// Memory Limit: 256
// Time Limit: 1000
// Start: Sat 28 Sep 2024 01:00:56 PM EEST 
#ifdef RAMEZ
#include "debug.hpp"
#else
#define debug(...) 0
...
void solve(){
  ...
  int timeWithout = t * ceil((n * 1.0)/k);

  int timeWith = 1, bakedWith = 0;
  while(true){
    if(timeWith%t == 0) bakedWith += k;
    if(timeWith > d && (timeWith-d)%t == 0) bakedWith += k;

    if(bakedWith < n) timeWith++;
    else break;
  }

  debug(timeWithout, timeWith);

  if(timeWith < timeWithout) cout << "YES\n";
  else cout << "NO\n";
}
