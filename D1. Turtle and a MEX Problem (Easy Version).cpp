...
// Memory Limit: 256
// Time Limit: 2000
// Start: Tue Aug 27 16:29:19 2024
#include <iostream>
#include <algorithm>
#include <cmath>
#include <climits>
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n ; i++) {
            int q; cin >> q;
            vll v(q); cin >> v;
            maxSecondMex = max(maxSecondMex, secondMex(v));
        }
        ll answer = min(maxSecondMex+1, m+1) * maxSecondMex;
        if(m > maxSecondMex) answer += sumOfAll1ToN(m) - sumOfAll1ToN(maxSecondMex);
